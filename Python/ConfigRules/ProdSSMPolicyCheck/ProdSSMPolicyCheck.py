...
DEFAULT_RESOURCE_TYPE = 'AWS::EC2::Account'
...
########################################################
# The VPC_TO_EVALUATE and ACCEPTED_SSM_POLICIES values #
# need to be updated depending on the environment.     #
########################################################

VPC_TO_EVALUATE = ['vpc-00bbef31e24687267']
ACCEPTED_SSM_POLICIES = ['prod-ssm-iam-policy']
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    reservation_details = ec2_client.describe_instances(InstanceIds=[configuration_item['resourceId']])['Reservations']
    print(configuration_item['resourceId'])
    for instances in reservation_details:
        instance_details = instances['Instances']
        for detail in instance_details:
            vpc_id = detail['VpcId']
            if vpc_id not in VPC_TO_EVALUATE:
                print('NOT_APPLICABLE')
                return build_evaluation_from_config_item(configuration_item, "NOT_APPLICABLE", 'Instance is not in production VPC. No need to evaluate.')
            else:
                iam_profile = detail['IamInstanceProfile']
                profile_arn = iam_profile.get('Arn')
                profile_name = re.search('instance-profile/(.*)', profile_arn).group(1)
                if len(profile_name) < 1:
                    print('NON_COMPLIANT')
                    return build_evaluation_from_config_item(configuration_item, "NON_COMPLIANT", 'No IAM profile found. Needs to have SSM policy attached.')
                else:
                    instance_profile = iam_client.get_instance_profile(InstanceProfileName=profile_name)['InstanceProfile']
                    instance_role = instance_profile.get('Roles')
                    for role_detail in instance_role:
                        role_name = role_detail['RoleName']
                        policy_names = iam_client.list_attached_role_policies(RoleName=role_name)['AttachedPolicies']
                        name_list = []
                        for name in policy_names:
                            specific_name = name['PolicyName']
                            name_list.append(specific_name)
                            print(name_list)
                    if any(list_name in name_list for list_name in ACCEPTED_SSM_POLICIES):
                        print('COMPLIANT')
                        return build_evaluation_from_config_item(configuration_item, "COMPLIANT", 'Instance already has prod SSM policy attached.')
                    else:
                        print('NON_COMPLIANT')
                        return build_evaluation_from_config_item(configuration_item, "NON_COMPLIANT", 'No IAM profile found. Needs to have SSM policy attached.')
