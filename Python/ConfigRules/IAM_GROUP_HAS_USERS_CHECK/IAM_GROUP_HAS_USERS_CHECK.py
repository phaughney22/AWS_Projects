...
'''
This AWS Config RDK rule checks for groups that have no users attached to them.

Groups without any users attached to them will be returned as 'NON-COMPLIANT'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    print('Config item group name is: ' + str(group_name))
    groups_user_data = iam_client.get_group(GroupName=group_name)['Users']
    print('Group user data is: ' + str(groups_user_data))
    if not groups_user_data:
        print('Returning empty')
        return build_evaluation_from_config_item(configuration_item, "NON_COMPLIANT", 'Non-compliant. No users are attached to this group.')
    if groups_user_data:
        print('Returning not empty')
        return build_evaluation_from_config_item(configuration_item, "COMPLIANT", 'Compliant. Users are attached to this group.')
    else:
        print('Returning anything else')
        return build_evaluation_from_config_item(configuration_item, "NON_APPLICABLE", 'Cannot determine compliance.')
